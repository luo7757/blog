            ...
            [`rows|${query.limit || 10}`] : [ //# 当前页列表数据
                {
                    ...
                    "thumb|1" : ["@image(300x250, @color, #fff, @name)"],
...
Mock.mock(/^\/api\/comment\/?(\?.+)?$/, 'get', function(options){// 分页获取评论
    const query = qs.parse(options.url);
...
Mock.mock('/api/comment', "post", function(){//提交评论 
    return Mock.mock({
        code : 0,
        msg : '',
        data : {
